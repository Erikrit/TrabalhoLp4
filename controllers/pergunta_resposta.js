module.exports = function(app){
     ...
     */
    app.get('/quiz/:id', function(req, resp){
        data = req.params;
        var con = app.persistencia.connectionFactory;
        var dao = new app.persistencia.pergunta_respostaDAO(con);
    ...
    */
    app.post('/quiz', function(req, resp){
         ...
         */
        var service = new app.services.pergunta_respostaDAO();
    ...
    */
    app.put('/quiz/:id', function(req, resp){
        ...
        dao.findById(param.id, function(exception, result){
            ...
            if(exception){
                resp.status(500);
                resp.send({"mensagem":"erro ao salvar quiz"});
                console.log(exception);
                return;
            }
            ...
            if(result.length == 0){
                resp.status(404);
                resp.send({"message":"quiz não encontrado"});
                return;
            }
    ...
    */
    app.delete('/quiz/:id', function(req, resp){
        dado = req.params;
        var con = app.persistencia.connectionFactory;
        var dao = new app.persistencia.pergunta_respostaDAO(con);
        ...
        dao.findById(dado.id, function(exception, result){
            ...
            dao.delete(dado.id, function(exception, result){
                ...
                if(exception){
                    resp.status(500);
                    resp.send({"mensagem":"erro ao deletar quiz"});
                    console.log(exception);
                    return;
                }
